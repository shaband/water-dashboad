<template>
    <vx-card :title="$t('users')">
        <template #actions>
            <div class="">
                <vs-button v-if="$can('Create User')" :to="{name:'admin.users.create'}" icon="add"></vs-button>
            </div>
        ...
        >
            <template slot="thead">
                ...
                <vs-th sort-key="email">{{ $t('Activation') }}</vs-th>
            ...
            <template slot-scope="{data}" class="mb-1">
                <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr,i) in data" :ref="`item-${tr.id}`">
                    ...
                    <vs-td :data="tr.id">
                        <div class="flex">
                            <vs-button v-if="$can('Edit User')"
                                       :to="{ name: 'admin.users.edit', params:{id:tr.id} }" flat icon="edit"
                                       class="mx-1">
                            </vs-button>
                            <vs-button v-if="$can('Delete User')" color="danger" type="filled" icon="delete"
...
export default {
    ...
    methods: {
        getDeliveries (page = 1) {
            user.all(page).then(({data: {data, links, meta}}) => {
                this.users = data
                this.meta = meta
            })
        },
        async deleteItem () {
            user.delete(this.selected.id).then((data) => {
                let deleted_index = this.users.findIndex(user => user.id === this.selected.id)
                this.users.splice(deleted_index, 1)
                this.$vs.notify({
        ...
        async toggle_activation (tr) {
           user.toggleBlock(tr.id).then(({data}) => {
