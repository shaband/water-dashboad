<template>
    <vx-card :title="$t('agents')">
        <template #actions>
            <div class="">
                <vs-button v-if="$can('Create Agent')" :to="{name:'admin.agents.create'}" icon="add"></vs-button>
            </div>
        ...
        >
            ...
            <template slot-scope="{data}" class="mb-1">
                <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr,i) in data" :ref="`item-${tr.id}`">
                    ...
                    <vs-td :data="tr.id">
                        <div class="flex">
                            <vs-button v-if="$can('Edit Agent')"
                                       :to="{ name: 'admin.agents.edit', params:{id:tr.id} }" flat icon="edit"
                                       class="mx-1">
                            </vs-button>
                            <vs-button v-if="$can('Delete Agent')" color="danger" type="filled" icon="delete"
...
export default {
    ...
    methods: {
        getDeliveries (page = 1) {
            agent.all(page).then(({data: {data, links, meta}}) => {
                this.agents = data
                this.meta = meta
            })
        },
        async deleteItem () {
            agent.delete(this.selected.id).then((data) => {
                let deleted_index = this.agents.findIndex(agent => agent.id === this.selected.id)
                this.agents.splice(deleted_index, 1)
                this.$vs.notify({
